...
object FPPractice {
    ...
    def passingStudents(grades: List[List[Int]]): Int = {
        grades.count(x => x.sum >= 5.75 && x.min > 4)
    }
    ...
    def headSumsTail(xs: List[List[Int]]): Option[Int] = {
        def check(xs: List[Int]): Boolean = xs match {
            case x :: tail => if (x == tail.sum) { true } else { false }
            case Nil => false
        }
        def find(xs: List[List[Int]]): Option[Int] = xs match {
            case Nil => None
            case x :: tail => if(check(x)) { Some(x.size) } else { find(tail) }
        }
        find(xs)
    }
}
